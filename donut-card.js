/*!
 * 🟢 Donut Card v22.0.0 (The Original Scale Fix)
 * - Fix: Schaal en tekstgrootte teruggezet naar origineel (Groot & Dik).
 * - Fix: Tekstkleur volgt thema via 'currentColor' zonder de grootte te slopen.
 */
...
(() => {
  ...
  class DonutCard extends HTMLElement {
    ...
    _buildDOM() {
      ...
      this.shadowRoot.innerHTML = `
        ...
        <ha-card>
          <div class="wrap">
            <svg viewBox="0 0 200 215" preserveAspectRatio="xMidYMid meet">
              <defs>
                <mask id="m">
              ...
              <text id="min-val" x="-10" y="210" class="corner" text-anchor="start"></text>
              <text id="max-val" x="210" y="210" class="corner" text-anchor="end"></text>
            </svg>
          </div>
        </ha-card>
      `;
    ...
    _updateValues() {
      ...
      if (c.show_trend && this._lastValue !== null) {
        ...
        if (Math.abs(diff) > val1 * 0.005) {
           this._elements.trend.textContent = diff > 0 ? "▲" : "▼";
           this._elements.trend.style.fill = diff > 0 ? "#00ff00" : "#ff4444"; // Groen bij stijging
        }
      ...
      if (c.entity_secondary && h.states[c.entity_secondary]) {
        ...
        this._elements.v1.setAttribute("y", "100");
        this._elements.trend.setAttribute("y", "100");
      } else {
        this._elements.v2.textContent = "";
        this._elements.v1.setAttribute("y", "118");
        this._elements.trend.setAttribute("y", "118");
      }

      const getVal = (id) => h.states[id] ? Number(h.states[id].state.replace(",",".")) : null;
      const minV = getVal(c.entity_min), maxV = getVal(c.entity_max);
  ...
  class DonutCardEditor extends HTMLElement {
    ...
    _build() {
      ...
      cp.innerHTML = `<style>.cp-panel { background: var(--secondary-background-color, rgba(150,150,150,0.1)); padding: 15px; border-radius: 12px; } .cp-row { display: flex; align-items: center; gap: 15px; margin-bottom: 10px; } .cp-color { width: 34px; height: 34px; border-radius: 50%; border: 2px solid rgba(128,128,128,0.3); cursor: pointer; -webkit-appearance: none; padding: 0; } .cp-label { font-size: 13px; font-weight: 500; width: 60px; }</style><div class="cp-panel"><strong>Kleurenverloop</strong><div class="cp-row" style="margin-top:10px;"><input type="color" class="cp-color" data-key="start_color"><span class="cp-label">Start (0%)</span></div>${[2,3,4,5].map(i => `<div class="cp-row"><input type="range" style="flex:1" data-key="stop_${i}" min="1" max="100"><input type="color" class="cp-color" data-key="color_${i}"><span id="perc_${i}" class="cp-label">0%</span></div>`).join('')}</div>`;
