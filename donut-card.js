/*!
 * Donut Card v1.9
 * Full file including robust Lovelace card-picker registration
 */
...
(() => {
  ...
  class DonutCard extends HTMLElement {
    ...
    _buildStops(c) {
      return [
        [0.0, this._sanitizeColor(c.start_color)],
  ...
  class DonutCardEditor extends HTMLElement {
    ...
    _sanitizeColor(color) {
      if (typeof color === "string" && /^#[0-9-A-F]{6}$/i.test(color.trim())) return color.trim();
      if (typeof color === "string" && /^#[0-9-A-F]{3}$/i.test(color.trim()))
        return "#" + color.substr(1).split("").map((s) => s + s).join("");
      return "#ff0000";
    }
    ...
    _updateFields() {
      ...
      [
        ...
        const el = this.querySelector(`[data-k="${key}"]`);
        if (el && el !== focused) {
          el.value = this._config[key] === undefined ? "" : this._config[key];
        }
      });
    }
  ...
  try {
    if (!customElements.get("donut-card")) {
      ...
      console.info(`DonutCard custom element registered`);
    }
    if (!customElements.get("donut-card-editor")) {
      customElements.define("donut-card-editor", DonutCardEditor);
      console.info(`DonutCardEditor custom element registered`);
    }
   ...
   // Card picker registration (ensures card appears in Lovelace picker)
  (function registerInPicker() {
    ...
    function register() {
      try {
        ...
        if (!exists) {
          ...
          console.info("Donut Card registered in picker");
        }
      } catch (e) {
        console.warn("Could not register in picker", e);
      }
    }
