/*!
 * 🟢 Donut Card v3.3.1 (Final Registration Fix)
 * Identieke registratie-methode als de werkende Battery Donut Card.
 */
...
(() => {
        ...
        <ha-card>
          <div class="wrap">
            <svg viewBox="0 0 260 260">
              ...
              <text x="${cx}" y="${cy - R - 25}" font-size="${R * 0.42}" font-weight="300" text-anchor="middle">${c.top_label_text}</text>
              <text id="val1" x="${cx}" y="${cy - R * 0.08}" font-size="${R * 0.3}" text-anchor="middle" font-weight="400">--</text>
              <text id="val2" x="${cx}" y="${cy + R * 0.35}" font-size="${R * 0.3}" text-anchor="middle" font-weight="300"></text>
            </svg>
          </div>
        </ha-card>
      `;
  ...
  if (!window.customCards.some(c => c.type === TAG)) {
    window.customCards.push({ type: TAG, name: "Generic Donut Card", description: "Gecentraliseerde donut kaart voor algemeen gebruik.", preview: true });
  }
})();
